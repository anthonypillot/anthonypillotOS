    ...
    >
        ...
        <span aria-hidden="true">
            <span
                ...
                :key="index"
                :class="
                    isRevealedOrDone(index) ? className : encryptedClassName
                "
            >
                {{ char }}
            </span>
...
const availableChars = computed<string[]>(() => {
    return props.useOriginalCharsOnly
        ? Array.from(new Set(props.text.split(""))).filter(
              (char) => char !== " ",
          )
        ...
        if (props.sequential) {
            if (direction.value === "forward") {
                ...
                if (pointerRef.value < orderRef.value.length) {
                    ...
                    const newRevealed = new Set(revealedIndices.value);
                    newRevealed.delete(idxToRemove ?? 0);
                    revealedIndices.value = newRevealed;
                    displayText.value = shuffleText(props.text, newRevealed);
            ...
            if (direction.value === "forward") {
                ...
                const removeCount = Math.max(
                    1,
                    Math.ceil(
                        props.text.length / Math.max(1, props.maxIterations),
                    ),
                );
                ...
                if (
                    nextSet.size === 0 ||
                    currentIteration >= props.maxIterations
                ) {
                    stopInterval();
                    isAnimating.value = false;
                    isDecrypted.value = false;
...
function isRevealedOrDone(index: number): boolean {
    return (
        revealedIndices.value.has(index) ||
        (!isAnimating.value && isDecrypted.value)
    );
}
