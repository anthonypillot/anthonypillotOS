...
import { clean } from "~/server/services/historyCleaner.service";
...
export default defineEventHandler(async (event): Promise<HistoryCleanerResult> => {
  ...
  try {
    const result = await clean(body.account, body.repository, body.token, body.options);
    logger.success(`History cleaner request processed successfully, result: ${JSON.stringify(result)}`);
    return {
      workflows: {
        complete: [],
        notFound: [],
